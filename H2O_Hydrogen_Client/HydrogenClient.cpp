...
void hydrogenSend(SOCKET clientSocket, int hydrogenInput) {

    for (int i = 1; i <= hydrogenInput; ++i) {
        ...
        std::string message = "H" + std::to_string(i) + ", request, " + timestamp;
...
int main() {
    ...
    if (connect(clientSocket, (SOCKADDR*)&clientService, sizeof(clientService)) == SOCKET_ERROR) {
        ...
        int hydrogenInput;
        std::cout << "Input number of Hydrogen bond requests: ";
        std::cin >> hydrogenInput;

        hydrogenSend(clientSocket, hydrogenInput);
